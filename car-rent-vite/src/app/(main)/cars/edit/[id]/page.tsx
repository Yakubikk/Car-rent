...
export default function EditCarPage() {
  ...
  // Загрузка данных автомобиля при монтировании компонента
  useEffect(() => {
    async function loadCar() {
      ...
      try {
        ...
        setError("Не удалось загрузить данные автомобиля. Пожалуйста, попробуйте снова позже.");
      } finally {
        setLoading(false);
      }
    }
  ...
  // Проверка разрешений
  if (!hasPermission(Permission.EDIT_CAR)) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white p-6 rounded-lg shadow-pink border border-pink-border">
          ...
          <p className="text-pink-text mb-4">У вас нет разрешения на редактирование автомобилей.</p>
          <button
            onClick={() => navigate(-1)}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Убедимся, что у нас есть полный объект Car с идентификатором
      if (!car.id) {
        throw new Error("Отсутствует идентификатор автомобиля");
      }
      
      await CarsApi.putCar(car as Car);
      ...
      setError("Не удалось обновить автомобиль. Пожалуйста, попробуйте снова позже.");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white p-6 rounded-lg shadow-pink border border-pink-border text-center">
          <h2 className="text-2xl font-bold text-pink-text mb-4">Автомобиль успешно обновлен!</h2>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-pink-text">Редактирование автомобиля</h1>
        <p className="text-sm text-pink-text mt-1">Внесите изменения в данные автомобиля</p>
      </div>
